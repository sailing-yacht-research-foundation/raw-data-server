  ...
  while (shouldContinue) {
    try {
      ...
      for (const race of races) {
        const objectToPass = {};
        const event = await db.tractracEvent.findOne({
          where: {
            id: race.event,
          },
          raw: true,
        ...
        if (event) {
          objectToPass.TracTracEvent = [event];
        }
        ...
        const controlPoints = await db.tractracControlPoint.findAll(raceFilter);
        ...
        const passings = await db.tractracCompetitorPassing.findAll(raceFilter);
