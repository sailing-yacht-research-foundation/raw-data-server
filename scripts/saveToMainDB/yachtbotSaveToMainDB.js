...
const mapAndSave = require('../../src/services/mappingsToSyrfDB/mapYachtbotToSyrf');

(async () => {
  const limit = 10;
  let page = 0;
  let shouldContinue = true;
  const existingData = await getExistingData(SOURCE.YACHTBOT);

  while (shouldContinue) {
    const races = await db.yachtBotRace.findAll({
      where: {
        original_id: {
    ...
    for (const race of races) {
      try {
        ...
        const boats = await db.yachtBotYacht.findAll(raceFilter);
        ...
        const positions = await db.yachtBotPosition.findAll(raceFilter);
        ...
        const marks = await db.yachtBotMark.findAll(raceFilter);
        const buoys = await db.yachtBotBuoy.findAll(raceFilter);
